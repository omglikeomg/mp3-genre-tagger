...
function findCanonicalGenre(genre) {
    const cleaned = genre.replace(/\s*\[DJ SET\]\s*$/i, '').trim();
    const match = VALID_GENRES.find(valid => valid.toLowerCase() === cleaned.toLowerCase());
...
function listReview() {
    ...
    console.log(`  node scripts/resolve-review.js --resolve <index> --primary "<genre>" [--secondary "<genre>"]`);
}
...
function resolveEntry(identifier, primaryGenre, secondaryGenre) {
    ...
    if (!isNaN(asNumber) && String(asNumber) === identifier) {
        ...
        if (asNumber < 0 || asNumber >= reviews.length) {
            console.error(`❌ Index ${asNumber} is out of range. There are ${reviews.length} entries (0–${reviews.length - 1}).`);
            process.exit(1);
        }
        ...
        const matches = reviews
            ...
            .filter(({ entry }) => entry.filepath && entry.filepath.includes(identifier));

        if (matches.length === 0) {
            console.error(`❌ No manual_review entry matches filepath "${identifier}".`);
            process.exit(1);
        }
        if (matches.length > 1) {
    ...
    console.log(`   Moved from manual_review[${targetIndex}] → operations[${data.operations.length - 1}]`);
