...
if (!fs.existsSync(PENDING_OPS_FILE)) {
  console.error(
    `❌  Could not find pending_operations.json at:\n    ${PENDING_OPS_FILE}`,
  );
  process.exit(1);
}
...
// Derive the base folder name from MUSIC_DIR so it matches what's in filepaths.
// e.g. "./Music" → "Music", "/home/user/Music" → "Music"
const MUSIC_FOLDER = path.basename(MUSIC_DIR);

function extractArtist(filepath) {
  // Normalise separators
  const parts = filepath.replace(/\\/g, "/").split("/");
  // parts[0] = music root folder (e.g. "Music"), parts[1] = artist folder, parts[2] = filename
  if (
    parts.length >= 3 &&
    parts[0].toLowerCase() === MUSIC_FOLDER.toLowerCase()
  ) {
    return parts[1];
  }
...
function genreMatches(op) {
  ...
  const inSecondary = (op.secondary_genre ?? "")
    .toLowerCase()
    .includes(queryLower);
...
const matchLabel =
  matchMode === "any" ? "primary or secondary" : `${matchMode} only`;
...
console.log(
  `🎤  Artists with ≤ ${maxTracks} track(s)  : ${thin.length}  (out of ${matchedTotal} total matched tracks)`,
);
console.log();

if (thin.length === 0) {
  console.log(
    "  ✅  No thin artists found — every artist has more tracks than the threshold.\n",
  );
  process.exit(0);
}
...
for (const [artist, stats] of thin) {
  const name =
    artist.length > colArtist ? artist.slice(0, colArtist - 1) + "…" : artist;
  console.log(
