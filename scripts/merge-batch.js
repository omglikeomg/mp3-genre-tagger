...
function validateBatchResult(data, filePath) {
  ...
  if (typeof data.batch !== "number") {
    warnings.push(
      '"batch" field is missing or not a number — LAST_BATCH will not be updated',
    );
  }
  ...
  (data.operations || []).forEach((op, i) => {
    ...
    if (op.primary_genre) {
      ...
      if (!canon) {
        warnings.push(
          `operations[${i}]: invalid primary_genre "${op.primary_genre}" — will be skipped`,
        );
        invalidGenreCount++;
      }
    } else {
    ...
    if (op.secondary_genre && !isValidGenre(op.secondary_genre)) {
      warnings.push(
        `operations[${i}]: invalid secondary_genre "${op.secondary_genre}" — will be cleared`,
      );
    }
  });
  ...
  if (data.artist_map_updates && typeof data.artist_map_updates === "object") {
    Object.entries(data.artist_map_updates).forEach(([artist, genres]) => {
      if (genres.primary_genre && !isValidGenre(genres.primary_genre)) {
        warnings.push(
          `artist_map_updates["${artist}"]: invalid primary_genre "${genres.primary_genre}"`,
        );
      }
      if (genres.secondary_genre && !isValidGenre(genres.secondary_genre)) {
        warnings.push(
          `artist_map_updates["${artist}"]: invalid secondary_genre "${genres.secondary_genre}"`,
        );
      }
    });
  }
...
function mergeOperations(existing, incoming) {
  ...
  for (const op of incoming) {
    // Canonicalize genres
    const canonPrimary = op.primary_genre
      ? canonicalGenre(op.primary_genre)
      : null;
    const canonSecondary =
      op.secondary_genre && isValidGenre(op.secondary_genre)
    ...
    if (!canonPrimary) {
      skipped.push({
        ...
        reason: `invalid primary_genre: "${op.primary_genre}"`,
      });
      continue;
    }
    ...
    if (existingPaths.has(op.filepath)) {
      ...
      existing[idx] = {
        ...op,
        primary_genre: canonPrimary,
        secondary_genre: canonSecondary,
      };
...
function mergeArtistMap(artistMap, updates) {
  ...
  for (const [artist, genres] of Object.entries(updates)) {
    const primaryOk =
      !genres.primary_genre || isValidGenre(genres.primary_genre);
    const secondaryOk =
      !genres.secondary_genre || isValidGenre(genres.secondary_genre);

    if (!primaryOk) {
      results.invalid.push({
        artist,
        reason: `invalid primary_genre: "${genres.primary_genre}"`,
      });
      continue;
    }
...
function main() {
  ...
  const batchNumber =
    typeof batchData.batch === "number" ? batchData.batch : null;
  ...
  console.log(
    `📊 Batch ${batchNumber ?? "(unknown)"}: ${incomingOps.length} operations, ${incomingReviews.length} manual_review, ${Object.keys(artistUpdates).length} artist_map updates`,
  );
  ...
  if (fs.existsSync(OPERATIONS_FILE)) {
    ...
    if (!Array.isArray(pendingData.manual_review))
      pendingData.manual_review = [];
  } else {
    console.log(`ℹ️  ${OPERATIONS_FILE} does not exist — will create it.`);
  }

  const artistMap = fs.existsSync(ARTIST_MAP_FILE)
    ? loadJson(ARTIST_MAP_FILE)
    : {};
  ...
  console.log(
    `   Added  : +${newOps}${overwritten > 0 ? ` (${overwritten} overwritten)` : ""}`,
  );
  if (skipped > 0) {
    console.log(`   Skipped: ${skipped} (invalid genres)`);
    opResult.skipped.forEach((s) =>
  ...
  console.log(
    `   Added  : +${reviewResult.added.filter((r) => !r.overwritten).length}`,
  );
  ...
  if (mapResult.updated.length > 0) {
    ...
    mapResult.updated.forEach((a) =>
      console.log(`     ✏️  ${a} -> ${artistUpdates[a].primary_genre}`),
    );
  }
  if (mapResult.added.length > 0) {
    console.log(`   Added  : ${mapResult.added.length}`);
    mapResult.added.forEach((a) =>
      console.log(`     ➕ ${a} -> ${artistUpdates[a].primary_genre}`),
    );
  }
  if (mapResult.invalid.length > 0) {
    console.log(`   Invalid: ${mapResult.invalid.length}`);
    mapResult.invalid.forEach((e) =>
      console.log(`     ❌ ${e.artist}: ${e.reason}`),
    );
  }
