...
// --- CONFIGURATION ---
const MUSIC_DIR = './Music'; // Adjust this to your actual Music path
const BATCH_SIZE = 10;
...
async function run() {
    ...
    const artistFolders = entries
        ...
        .map(e => ({ name: e.name, path: path.join(MUSIC_DIR, e.name), isComp: false }));

    const compilationFolders = [];
    const compBase = path.join(MUSIC_DIR, 'Compilations');
    if (fs.existsSync(compBase)) {
        const subDirs = fs.readdirSync(compBase, { withFileTypes: true })
            .filter(e => e.isDirectory())
            .map(e => ({ name: e.name, path: path.join(compBase, e.name), isComp: true }));
